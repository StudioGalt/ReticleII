...
class ReticleOverlay:
            ...
            with self.vp_win.frame:
                with ui.ZStack():
                    ...
                    with self.scene_view.scene:
                        ...
                        elif self.model.composition_mode.as_int == CompositionGuidelines.BigX:
                            self._build_BigX()
                        elif self.model.composition_mode.as_int == CompositionGuidelines.BigCirclet:
                            self._build_BigCirclet()
                        ...
                        if self.model.circlet_action_safe_enabled.as_bool:
                            self._build_safe_circlet(self.model.circlet_action_safe_percentage.as_float / 100.0,
                                                  color=cl.circlet_action_safe_default)
                        if self.model.circlet_title_safe_enabled.as_bool:
                            self._build_safe_circlet(self.model.circlet_title_safe_percentage.as_float / 100.0,
                                                  color=cl.circlet_title_safe_default)
                        if self.model.circlet_custom_safe_enabled.as_bool:
                            self._build_safe_circlet(self.model.circlet_custom_safe_percentage.as_float / 100.0,
                                                  color=cl.circlet_custom_safe_default)
    ...
    def _build_BigX(self):
        """Build the scene ui graphics for the BigX composition mode."""
        aspect_ratio = self.get_aspect_ratio()
        line_color = cl.comp_lines_default
        inverse_ratio = 1 / aspect_ratio
        if self.scene_view.aspect_ratio_policy == scene.AspectRatioPolicy.PRESERVE_ASPECT_VERTICAL:
            scene.Line([aspect_ratio, 1, 0], [-aspect_ratio, -1, 0], color=line_color)
            scene.Line([-aspect_ratio, 1, 0], [aspect_ratio, -1, 0], color=line_color)
        else:
            scene.Line([-1, -inverse_ratio, 0], [1, inverse_ratio, 0], color=line_color)
            scene.Line([-1, inverse_ratio, 0], [1, -inverse_ratio, 0], color=line_color)

    def _build_BigCirclet(self):
        """Build the scene ui graphics for the Circlet composition mode."""
        aspect_ratio = self.get_aspect_ratio()
        line_color = cl.comp_lines_default
        inverse_ratio = 1 / aspect_ratio
        if self.scene_view.aspect_ratio_policy == scene.AspectRatioPolicy.PRESERVE_ASPECT_VERTICAL:
            scene.Arc(0.80, begin= 0, end=2*pi, thickness=1, wireframe=True, color=line_color) 
        else:
            scene.Arc(0.80, begin= 0, end=2*pi, thickness=1, wireframe=True, color=line_color) 
    ...
    def _build_safe_circlet(self, percentage, color):
        """Build the scene ui graphics for the smaller Circlets
        ...
        inverse_ratio = 1 / aspect_ratio
        if self.scene_view.aspect_ratio_policy == scene.AspectRatioPolicy.PRESERVE_ASPECT_VERTICAL:
            scene.Arc(percentage, begin= 0, end=2*pi, thickness=1, wireframe=True, color=color) 
        else:
            scene.Arc(percentage, begin= 0, end=2*pi, thickness=1, wireframe=True, color=color) 
...
class ReticleMenu:
    ...
    def on_composition_mode_changed(self, guideline_type):
        ...
        self.comp_BigX_button.checked = guideline_type == CompositionGuidelines.BigX
        self.comp_BigCirclet_button.checked = guideline_type == CompositionGuidelines.BigCirclet
    ...
    def show_reticle_menu(self, x, y, button, modifier):
        ...
        with self.reticle_menu:
            with ui.Frame(width=0, height=150):
                with ui.HStack():
                    with ui.VStack():
                        ...
                        with ui.VGrid(style=styles.comp_group_style, width=150, height=0,
                            ...
                            with ui.HStack():
                                ...
                                self.comp_crosshair_button = ui.Button("Crosshair", name="Crosshair",
                                                                       ...
                                                                       clicked_fn=callback)
                            with ui.HStack():
                                BigX_checked = current_comp_mode == CompositionGuidelines.BigX
                                callback = partial(self.on_composition_mode_changed, CompositionGuidelines.BigX)
                                self.comp_BigX_button = ui.Button("BigX", name="BigX", checked=BigX_checked,
                                                                  width=70, height=70, clicked_fn=callback)
                            with ui.HStack():
                                BigCirclet_checked = current_comp_mode == CompositionGuidelines.BigCirclet
                                callback = partial(self.on_composition_mode_changed, CompositionGuidelines.BigCirclet)
                                self.comp_BigCirclet_button = ui.Button("BigCirclet", name="BigCirclet", checked=BigCirclet_checked,
                                                                  width=70, height=70, clicked_fn=callback)
                    ...
                    with ui.VStack(style=styles.safe_areas_group_style):
                        ...
                        with ui.HStack(width=0):
                            ui.Spacer(width=20)
                            cb = ui.CheckBox(model=self.model.circlet_action_safe_enabled)
                            circlet_action_safe_group = ui.HStack(enabled=self.model.circlet_action_safe_enabled.as_bool)
                            callback = partial(self.on_group_check_changed, circlet_action_safe_group)
                            cb.model.add_value_changed_fn(callback)
                            with circlet_action_safe_group:
                                ui.Spacer(width=10)
                                ui.Label("Circle Action Safe", alignment=ui.Alignment.TOP)
                                ui.Spacer(width=14)
                                with ui.VStack():
                                    ui.FloatSlider(self.model.circlet_action_safe_percentage, width=100,
                                                   format="%.0f%%", min=0, max=100, step=1)
                                    ui.Rectangle(name="CircleActionSwatch", height=5)
                                    ui.Spacer()

                        with ui.HStack(width=0):
                            ui.Spacer(width=20)
                            cb = ui.CheckBox(model=self.model.circlet_title_safe_enabled)
                            circlet_title_safe_group = ui.HStack(enabled=self.model.circlet_title_safe_enabled.as_bool)
                            callback = partial(self.on_group_check_changed, circlet_title_safe_group)
                            cb.model.add_value_changed_fn(callback)
                            with circlet_title_safe_group:
                                ui.Spacer(width=10)
                                ui.Label("Circle Title Safe", alignment=ui.Alignment.TOP)
                                ui.Spacer(width=14)
                                with ui.VStack():
                                    ui.FloatSlider(self.model.circlet_title_safe_percentage, width=100,
                                                   format="%.0f%%", min=0, max=100, step=1)
                                    ui.Rectangle(name="CircleTitleSwatch", height=5)
                                    ui.Spacer()

                        with ui.HStack(width=0):
                            ui.Spacer(width=20)
                            cb = ui.CheckBox(model=self.model.circlet_custom_safe_enabled)
                            circlet_custom_safe_group = ui.HStack(enabled=self.model.circlet_custom_safe_enabled.as_bool)
                            callback = partial(self.on_group_check_changed, circlet_custom_safe_group)
                            cb.model.add_value_changed_fn(callback)
                            with circlet_custom_safe_group:
                                ui.Spacer(width=10)
                                ui.Label("Circle Custom Safe", alignment=ui.Alignment.TOP)
                                ui.Spacer(width=14)
                                with ui.VStack():
                                    ui.FloatSlider(self.model.circlet_custom_safe_percentage, width=100,
                                                   format="%.0f%%", min=0, max=100, step=1)
                                    ui.Rectangle(name="CircleCustomSwatch", height=5)
                                    ui.Spacer()
